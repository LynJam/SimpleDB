...
/**
 * BTreeFile is an implementation of a DbFile that stores a B+ tree. Specifically, it stores a pointer to a root page, a set of internal pages, and a set of leaf pages, which contain a collection of
 * tuples in sorted order. BTreeFile works closely with BTreeLeafPage, BTreeInternalPage, and BTreeRootPtrPage. The format of these pages is described in their constructors.
 *
 * @author Becca Taft
...
public class BTreeFile implements DbFile {
    ...
    public BTreeFile(File f, int key, TupleDesc td) {
        this.f = f;
        this.tableid = f.getAbsoluteFile()
            .hashCode();
        this.keyField = key;
        this.td = td;
    }
    ...
    /**
     * Returns an ID uniquely identifying this BTreeFile. Implementation note: you will need to generate this tableid somewhere and ensure that each BTreeFile has a "unique id," and that you always
     * return the same value for a particular BTreeFile. We suggest hashing the absolute file name of the file underlying the BTreeFile, i.e. f.getAbsoluteFile().hashCode().
     *
    ...
    /**
     * Read a page from the file on disk. This should not be called directly but should be called from the BufferPool via getPage()
     *
    ...
    /**
     * Write a page to disk.  This should not be called directly but should be called from the BufferPool when pages are flushed to disk
     *
    ...
    public void writePage(Page page) throws IOException {
        ...
        if (id.pgcateg() == BTreePageId.ROOT_PTR) {
            ...
            rf.seek(BTreeRootPtrPage.getPageSize() + (long) (page.getId()
    ...
    /**
     * Recursive function which finds and locks the leaf page in the B+ tree corresponding to the left-most page possibly containing the key field f. It locks all internal nodes along the path to the
     * leaf node with READ_ONLY permission, and locks the leaf node with permission perm.
     *
    ...
    /**
     * Split a leaf page to make room for new tuples and recursively split the parent node as needed to accommodate a new entry. The new entry should have a key matching the key field of the first
     * tuple in the right-hand page (the key is "copied up"), and child pointers pointing to the two leaf pages resulting from the split.  Update sibling pointers and parent pointers as needed.
     *
     ...
     * @param field - the key field of the tuple to be inserted after the split is complete. Necessary to know which of the two pages to return.
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field) throws DbException, IOException, TransactionAbortedException {
        ...
        // 链表插入一个新节点 newRightPage 后，修改双链表的指针指向
        BTreePageId oldRightPageId = page.getRightSiblingId();
        BTreeLeafPage oldRightPage = oldRightPageId != null ? (BTreeLeafPage) getPage(tid, dirtypages, oldRightPageId, Permissions.READ_WRITE) : null;
        ...
        // 获取指向page的内部节点，并创建一个新的 entity 指向 page 和 newRightPage
        BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
        Field mid = newRightPage.iterator()
            .next()
            .getField(keyField);
        BTreeEntry bTreeEntry = new BTreeEntry(mid, page.getId(), newRightPage.getId());
    ...
    /**
     * Split an internal page to make room for new entries and recursively split its parent page as needed to accommodate a new entry. The new entry for the parent should have a key matching the
     * middle key in the original internal page being split (this key is "pushed up" to the parent). The child pointers of the new parent entry should point to the two internal pages resulting from
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeInternalPage page, Field field) throws DbException, IOException, TransactionAbortedException {
        ...
        BTreeEntry mid = it.next(); // 选出中间的节点，上移作为父节点
        ...
        if (field.compare(Op.GREATER_THAN, mid.getKey())) {
            return newRightPage;
        } else if (field.compare(Op.EQUALS, mid.getKey())) {
            return parent;
        } else {
            return page;
        }
    ...
    /**
     * Method to encapsulate the process of getting a parent page ready to accept new entries. This may mean creating a page to become the new root of the tree, splitting the existing parent page if
     * there are no empty slots, or simply locking and returning the existing parent page.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
     * @param parentId - the id of the parent. May be an internal page or the RootPtr page
     * @param field - the key of the entry which will be inserted. Needed in case the parent must be split to accommodate the new entry
    ...
    /**
     * Method to encapsulate the process of locking/fetching a page.  First the method checks the local cache ("dirtypages"), and if it can't find the requested page there, it fetches it from the
     * buffer pool. It also adds pages to the dirtypages cache if they are fetched with read-write permission, since presumably they will soon be dirtied by this transaction.
     *
    ...
    /**
     * Insert a tuple into this BTreeFile, keeping the tuples in sorted order. May cause pages to split if the page where tuple t belongs is full.
     *
     * @param tid - the transaction id
     * @param t - the tuple to insert
     * @return a list of all pages that were dirtied by this operation. Could include many pages since parent pointers will need to be updated when an internal node splits.
    ...
    /**
     * Handle the case when a B+ tree page becomes less than half full due to deletions. If one of its siblings has extra tuples/entries, redistribute those tuples/entries. Otherwise merge with one of
     * the siblings. Update pointers as needed.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
    ...
    /**
     * Handle the case when a leaf page becomes less than half full due to deletions. If one of its siblings has extra tuples, redistribute those tuples. Otherwise merge with one of the siblings.
    ...
    /**
     * Steal tuples from a sibling and copy them to the given page so that both pages are at least half full.  Update the parent's entry so that the key matches the key field of the first tuple in the
     * right-hand page.
     *
     * @param page - the leaf page which is less than half full
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling, BTreeInternalPage parent, BTreeEntry entry, boolean isRightSibling) throws DbException {
        ...
        int targetTuplesNum = (tuplesNum + siblingTuplesNum) / 2;
        while (tuplesNum < targetTuplesNum) {
            Tuple next = it.next();
    ...
    /**
     * Handle the case when an internal page becomes less than half full due to deletions. If one of its siblings has extra entries, redistribute those entries. Otherwise merge with one of the
    ...
    /**
     * Steal entries from the left sibling and copy them to the given page so that both pages are at least half full. Keys can be thought of as rotating through the parent entry, so the original key
     * in the parent is "pulled down" to the right-hand page, and the last key in the left-hand page is "pushed up" to the parent.  Update parent pointers as needed.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
     * @param page - the internal page which is less than half full
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeInternalPage page, BTreeInternalPage leftSibling, BTreeInternalPage parent, BTreeEntry parentEntry)
        ...
        int pageEntityNum = page.getNumEntries();
        int siblingEntityNum = leftSibling.getNumEntries();
        int targetNum = (pageEntityNum + siblingEntityNum) / 2;
        BTreeEntry entry = it.next();
        BTreeEntry mid = new BTreeEntry(parentEntry.getKey(), entry.getRightChild(), page.iterator()
            .next()
            .getLeftChild());
        page.insertEntry(mid);
        pageEntityNum++;
        while (pageEntityNum < targetNum) {
    ...
    /**
     * Steal entries from the right sibling and copy them to the given page so that both pages are at least half full. Keys can be thought of as rotating through the parent entry, so the original key
     * in the parent is "pulled down" to the left-hand page, and the last key in the right-hand page is "pushed up" to the parent.  Update parent pointers as needed.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
     * @param page - the internal page which is less than half full
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeInternalPage page, BTreeInternalPage rightSibling, BTreeInternalPage parent, BTreeEntry parentEntry)
        ...
        // that the entries are evenly distributed. Be sure to update
        // the corresponding parent entry. Be sure to update the parent
        // pointers of all children in the entries that were moved.Iterator<BTreeEntry> it = leftSibling.reverseIterator();
        Iterator<BTreeEntry> it = rightSibling.iterator();
        int pageEntityNum = page.getNumEntries();
        int siblingEntityNum = rightSibling.getNumEntries();
        int targetNum = (pageEntityNum + siblingEntityNum) / 2;
        BTreeEntry entry = it.next();
        BTreeEntry mid = new BTreeEntry(parentEntry.getKey(), page.reverseIterator()
            .next()
            .getRightChild(), entry.getLeftChild());
        page.insertEntry(mid);
        pageEntityNum++;
        while (pageEntityNum < targetNum) {
    ...
    /**
     * Merge two leaf pages by moving all tuples from the right page to the left page. Delete the corresponding key and right child pointer from the parent, and recursively handle the case when the
     * parent gets below minimum occupancy. Update sibling pointers as needed, and make the right page available for reuse.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
    ...
    /**
     * Merge two internal pages by moving all entries from the right page to the left page and "pulling down" the corresponding key from the parent entry. Delete the corresponding key and right child
     * pointer from the parent, and recursively handle the case when the parent gets below minimum occupancy. Update parent pointers as needed, and make the right page available for reuse.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
    ...
    /**
     * Method to encapsulate the process of deleting an entry (specifically the key and right child) from a parent node.  If the parent becomes empty (no keys remaining), that indicates that it was
     * the root node and should be replaced by its one remaining child.  Otherwise, if it gets below minimum occupancy for non-root internal nodes, it should steal from one of its siblings or merge
     * with a sibling.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
    ...
    private void deleteParentEntry(TransactionId tid, Map<PageId, Page> dirtypages, BTreePage leftPage, BTreeInternalPage parent, BTreeEntry parentEntry)
        ...
        if (parent.getNumEmptySlots() == parent.getMaxEntries()) {
            ...
            setEmptyPage(tid, dirtypages, parent.getId()
                .getPageNumber());
    ...
    /**
     * Delete a tuple from this BTreeFile. May cause pages to merge or redistribute entries/tuples if the pages become less than half full.
     *
     * @param tid - the transaction id
     * @param t - the tuple to delete
     * @return a list of all pages that were dirtied by this operation. Could include many pages since parent pointers will need to be updated when an internal node merges.
     * @see #handleMinOccupancyPage(TransactionId, Map, BTreePage)
     */
    public List<Page> deleteTuple(TransactionId tid, Tuple t) throws DbException, IOException, TransactionAbortedException {
        Map<PageId, Page> dirtypages = new HashMap<>();

        BTreePageId pageId = new BTreePageId(tableid, t.getRecordId()
            .getPageId()
            .getPageNumber(), BTreePageId.LEAF);
    ...
    /**
     * Get the page number of the first empty page in this BTreeFile. Creates a new page if none of the existing pages are empty.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
    ...
    /**
     * Method to encapsulate the process of creating a new page.  It reuses old pages if possible, and creates a new page if none are available.  It wipes the page on disk and in the cache and returns
     * a clean copy locked with read-write permission
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
    ...
    /**
     * Mark a page in this BTreeFile as empty. Find the corresponding header page (create it if needed), and mark the corresponding slot in the header page as empty.
     *
     * @param tid - the transaction id
     * @param dirtypages - the list of dirty pages which should be updated with all new dirty pages
...
//		}
    ...
    /**
     * get the specified tuples from the file based on its IndexPredicate value on behalf of the specified transaction. This method will acquire a read lock on the affected pages of the file, and may
    ...
    /**
     * Get an iterator for all tuples in this B+ tree file in sorted order. This method will acquire a read lock on the affected pages of the file, and may block until the lock can be acquired.
     *
     * @param tid - the transaction id
    ...
    protected Tuple readNext() throws TransactionAbortedException, DbException {
        ...
        while (it == null && curp != null) {
            ...
            if (nextp == null) {
                ...
                curp = (BTreeLeafPage) Database.getBufferPool()
                    .getPage(tid, nextp, Permissions.READ_ONLY);
                it = curp.iterator();
                if (!it.hasNext()) {
                    it = null;
                }
...
class BTreeSearchIterator extends AbstractDbFileIterator {
    ...
    protected Tuple readNext() throws TransactionAbortedException, DbException, NoSuchElementException {
        while (it != null) {
            ...
            while (it.hasNext()) {
                Tuple t = it.next();
                if (t.getField(f.keyField())
                    .compare(ipred.getOp(), ipred.getField())) {
                    return t;
