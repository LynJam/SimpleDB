...
public class HeapPage implements Page {
    ...
    final TupleDesc td;
    /**
     * header是每个tuple slot的bitmap。如果bitmap中对应的某个tuple的bit是1，则这个tuple是有效的，否则无效（被删除或者没被初始化）
     */
    final byte[] header;
    final Tuple[] tuples;
    final int numSlots;
    ...
    /**
     * Create a HeapPage from a set of bytes of data read from disk. The format of a HeapPage is a set of header bytes indicating the slots of the page that are in use, some number of tuple slots.
     * Specifically, the number of tuples is equal to: <p> floor((BufferPool.getPageSize()*8) / (tuple size * 8 + 1))
     * <p> where tuple size is the size of tuples in this
     * database table, which can be determined via {@link Catalog#getTupleDesc}. The number of 8-bit header words is equal to:
     * <p>
    ...
    public HeapPage(HeapPageId id, byte[] data) throws IOException {
        this.pid = id;
        this.td = Database.getCatalog()
            .getTupleDesc(id.getTableId());
    ...
    /**
     * Static method to generate a byte array corresponding to an empty HeapPage. Used to add new, empty pages to the file. Passing the results of this method to the HeapPage constructor will create a
     * HeapPage with no valid tuples in it.
     *
